...
class EkobaudatMaterial(models.Model):
    """Importierte ÖKOBAUDAT-Einträge (Ökobilanz-Datenbank des BMWSB).

    Hinweis: Die ÖKOBAUDAT enthält Umweltindikatoren (GWP, PENRT, …) und
    physikalische Bezugsgrößen (Rohdichte, Bezugseinheit) – KEINE U-/λ-Werte.
    Wärmeschutz-Kennwerte kommen aus DIN 4108-4 (siehe Presets im ViewSet).
    """
    ...
    embodied_co2 = models.FloatField(null=True, blank=True,
                                     help_text="GWP A1–A3 je kg, sofern umrechenbar [kg CO₂e/kg]")
    notes = models.TextField(blank=True)

    # --- ÖKOBAUDAT-Originalfelder (Export OBD_2024) ---
    uuid = models.CharField(max_length=36, blank=True, null=True, unique=True)
    dataset_type = models.CharField(max_length=50, blank=True)        # average/specific/generic …
    ref_quantity = models.FloatField(null=True, blank=True)           # Bezugsgröße (z. B. 1)
    ref_unit = models.CharField(max_length=20, blank=True)            # Bezugseinheit (m3, kg, qm …)
    density = models.FloatField(null=True, blank=True)                # Rohdichte [kg/m³]
    gwp_a1a3 = models.FloatField(null=True, blank=True)               # GWP Modul A1–A3 je Bezugseinheit
